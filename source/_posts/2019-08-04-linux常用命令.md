...
| -t   | --field-separator=SEP   | 指定分隔符，如果是`\t`，需要这样`sort -t$'\t' file`。**默认的分隔符为空白字符和非空白字符之间的空字符**，**并非网上众多文章所说的空格或制表符**(By default, fields are separated by the empty string between a non-blank character and a blank character.)**。** |
...
>   " foo bar"默认将分隔为两个字段" foo"和" bar"，而使用空格作为分隔符时将分隔为三个字段：第一个字段为空，第二个字段和第三个字段为"foo"和"bar"。使用下面三个sort语句可以验证默认的分隔符并非空格。
>
>   ```shell
